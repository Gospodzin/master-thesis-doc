...
\definition{gęstościowa osiągalność (ang. density reachability) (\myhyperref{fig:density-reachablity-reachability}{rysunek})}\newline
...
\definition{punkt szumu (ang. noise point)}\newline
...
\definition{odległość euklidesowa ($ d_e $)}
\begin{equation}
	d_e(p, q) = \sqrt{p \cdot q}
\end{equation}
\definition{odległość Manhattan ($ d_m $)}
\begin{equation}
d_m(p, q) = \sum_i^{n}{\abs{p_i-q_i}} \;:\; p,q\in \mathcal{R}^n
\end{equation}
\definition{podobieństwo kosinusowe ($ s_{cos} $)}
\begin{equation}
