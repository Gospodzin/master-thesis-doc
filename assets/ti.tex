...
\end{equation}
Niestety skuteczność tej metody jest podatna na rozkład punktów w zbiorze danych. \myhyperref{img:ti-max-ref}{Rysunki} przedstawiają sytuacje dla różnych rozkładów punktów. Na \myhyperref{img:ti-max-ref-good}{Rysunku} położenie punktu $ r $ jest korzystniejsze i wartość $ m $ jest większa niż na \myhyperref{img:ti-max-ref-bad}{Rysunku}. W pewnym stopniu można sobie z tym problemem poradzić, wyznaczając punkty referencyjne, dla różnych kombinacji minimum lub maksimum dla poszczególnych współrzędnych (np. $ r=(a_{1max}, a_{2min}, a_{3min}, \dots) $). Następnie, dla każdego z punktów referencyjnych można wyznaczyć $ m $ i wybrać $ r $, dla którego $ m $ jest największe. Wyznaczenie wartości $ m $ dla danego $ r $ jest realizowalne w czasie liniowym, ale wygenerowanych punktów referencyjnych może być $ 2^{dim(D)} $. Aby uniknąć problemów wydajnościowych w przypadku wysokowymiarowych danych można się ograniczyć do $ k $ spośród wygenerowanych punktów referencyjnych. 
